Blopnote allows to type a song translation from one language to another.

# How to use:
1. Pick a directory where translations should be saved
<img width="945" height="536" alt="image" src="https://github.com/user-attachments/assets/c05159b7-ffb8-4652-9958-d51f5d7663e1" />
<br>
<br>

2. Create new file
<img width="237" height="130" alt="image" src="https://github.com/user-attachments/assets/1d675133-b8ae-419b-9e76-b16767dfa2c0" />
<br>
<br>

3. Fill song info
<img width="742" height="748" alt="image" src="https://github.com/user-attachments/assets/b2323aff-14ff-45e0-88c3-5c503c63e4a7" />
<br>
<br>

4. Translate!
<img width="943" height="538" alt="image" src="https://github.com/user-attachments/assets/ded52912-9109-433f-9f1a-a16268989833" />
<br>
<br>

# Incredible features & why should you use `Blopnote` instead of standard Notepad:

- Auto-pulling of the lyrics from `Genius.com` in one click:
<img width="720" height="720" alt="lyrics_parsing_from_genius_com" src="https://github.com/user-attachments/assets/594ecc80-d7b8-4fe2-89ae-263c3894e15e" />
<br>
Selenium does all the work under the hood.
<br>
<br>

- Your translation and lyrics are in one convenient window (you can hide lyrics)

- Auto-filling of song's structure lines: empty lines, lines which are bracketed by `[]` (`genius.com`-oriented feature):
<img width="1280" height="720" alt="2026-08-25-15-22-34-_online-video-cutter com_" src="https://github.com/user-attachments/assets/cf55fadc-ae32-4860-98e9-a77fcd027110" />
<br>
<br>

- Auto-completion of already translated lines. You don't need to copy them manually.
<img width="1280" height="720" alt="auto_completion_feature" src="https://github.com/user-attachments/assets/c862001d-e477-4d38-9d41-248a99823af2" />
<br>
<br>

- You'll never loose the focus because the current line is highlighted with blue color
<img width="1280" height="720" alt="blue_line_feature" src="https://github.com/user-attachments/assets/80911fcf-eca4-4d80-9554-5debcaf9d64d" />
<br>
<br>

- Have forgotten some word's translation and web translate is far from you? Just hold `TAB` to skim the translation of current line pulled from google translate:
<img width="1440" height="720" alt="translation_skimming" src="https://github.com/user-attachments/assets/81ec9744-46fe-4a43-9750-35a140b30459" />
Selenium does this on song creation:
  1. Takes the lyrics of song.
  2. Pasts it to google translate.
  3. Parses the whole translation.
So it's shown instantly. Be sceptical about this machine translation.
<br>
<br>

- Autosave. No need in annoying `Ctrl + S` pressing after each word like in default Notepad.

# Hotkeys:
- `Ctrl + C` to copy current line
- `Ctrl + W` to close app (like in standard windows' notepad)

# Technology stack
- C#
- Windows.Forms
- MySQL
- Selenium

# More details:
User can past own lyrics (preferable ones are from https://genius.com), or app may parse lyrics automatically by built-in selenium parser.
In other words, this is a notepad-like app but with auto-save (each 5 seconds under the hood) and with lyrics in right part of screen. 
If somebody doesn't know, song translation is fun and it's not discussed.
